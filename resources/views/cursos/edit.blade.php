        ...
        <form action="{{ route('cursos.update', $curso) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="creditos" id="creditos" value="{{ old('creditos', $curso->creditos) }}" min="1" max="10"
                ...
                <div>
                    ...
                    <select name="facultad_id" id="facultad_id" 
                        ...
                        @foreach($facultades as $facultad)
                            <option value="{{ $facultad->id }}" {{ old('facultad_id', $curso->facultad_id) == $facultad->id ? 'selected' : '' }}>
                                {{ $facultad->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="docente" id="docente" value="{{ old('docente', $curso->docente) }}" 
            ...
            <div class="mt-6">
                ...
                <textarea name="descripcion" id="descripcion" rows="4" 
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $curso->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                @if($curso->imagen)
                    <img src="{{ asset('storage/' . $curso->imagen) }}" alt="{{ $curso->nombre }}" class="w-32 h-32 object-cover rounded mb-4">
                @else
